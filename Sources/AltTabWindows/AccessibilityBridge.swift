...
final class AccessibilityBridge {
    ...
    func currentFocusedWindowIdentity() -> WindowIdentity? {
        currentFocusedWindowContext()?.identity
    }
        ...
        for delay in retryDelays {
            ...
            if boolAttribute(kAXMinimizedAttribute as CFString, from: window) == true {
                ...
                if !isAcceptable(unminimizeResult) {
                    if isInvalidWindowError(unminimizeResult) {
            ...
            let allAcceptable =
                isAcceptable(frontmostResult) &&
                isAcceptable(mainResult) &&
                isAcceptable(focusedResult) &&
                isAcceptable(raiseResult)
    ...
    private func pointAttribute(_ attribute: CFString, from element: AXUIElement) -> CGPoint? {
        ...
        let extracted = AXValueGetValue(axValue, .cgPoint, &point)
        return extracted ? point : nil
    }
    ...
    private func sizeAttribute(_ attribute: CFString, from element: AXUIElement) -> CGSize? {
        ...
        let extracted = AXValueGetValue(axValue, .cgSize, &size)
    ...
    private func isAcceptable(_ error: AXError) -> Bool {
        switch error {
        case .success, .attributeUnsupported, .actionUnsupported:
            return true
        default:
            return false
        }
    ...
    private func isInvalidWindowError(_ error: AXError) -> Bool {
        switch error {
        case .invalidUIElement, .illegalArgument:
            return true
        default:
            return false
        }
