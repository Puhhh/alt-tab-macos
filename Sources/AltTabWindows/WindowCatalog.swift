...
final class WindowCatalog {
    ...
    private func makeEntry(
        ...
        if rawTitle.isEmpty, !matchedAXWindow.title.isEmpty {
            resolvedTitle = matchedAXWindow.title
        } else {
    ...
    private func bestMatch(
        ...
        let rankedCandidates = snapshots
            .map { snapshot in
                (
                    snapshot: snapshot,
                    score: score(for: snapshot, normalizedTitle: normalizedTitle, bounds: bounds)
                )
            }
            .sorted { lhs, rhs in
                lhs.score < rhs.score
        ...
        if rigour == .activation,
           ...
           abs(secondCandidate.score - bestCandidate.score) <= ambiguityMargin {
            let bestSignature = FrameSignature(bestCandidate.snapshot.frame)
            let secondSignature = FrameSignature(secondCandidate.snapshot.frame)
            if bestSignature == secondSignature {
                return nil
            }
        }
    ...
    private func score(
        ...
        if normalizedTitle.isEmpty || normalizedCandidate.isEmpty {
    ...
    private func refreshedAXWindow(for entry: WindowEntry) -> AccessibilityBridge.AXWindowSnapshot? {
        ...
        let storedSnapshot = AccessibilityBridge.AXWindowSnapshot(
            element: entry.axWindow,
            title: entry.title,
            frame: entry.frame
        )
